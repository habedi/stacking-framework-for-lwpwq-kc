...
if global_settings.use_stratified_folds or True:
    skf = StratifiedKFold(n_splits=global_settings.l3_n_folds, shuffle=True,
                          random_state=global_settings.global_seed + global_settings.random_seed_offsets[0])
else:
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(oofs_train_df, y_as_str)):
    ...
    model = cb.CatBoostRegressor(iterations=2000,
                                 ...
                                 depth=5,
                                 eval_metric='RMSE',
                                 random_seed=global_settings.global_seed + global_settings.random_seed_offsets[0],
...
inferences_df.to_csv(global_settings.get_meta_model_oofs_dir() / 'submission_cb.csv.gz', index=True)
...
meta_model_df.to_csv(global_settings.get_meta_model_oofs_dir() / f'val_cb.csv.gz', index=False)
