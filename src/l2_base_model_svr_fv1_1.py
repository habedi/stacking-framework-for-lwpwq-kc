...
train_x_y = pd.read_csv(global_settings.raw_features_dir /
                        f'train_features_with_scores_v{1}.csv.gz')
test_x = pd.read_csv(global_settings.raw_features_dir /
                     f'test_features_v{1}.csv.gz')
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(train_x, train_y_as_str)):
    ...
    model = make_pipeline(SimpleImputer(strategy='constant', fill_value=-1),
                          MinMaxScaler(),
                          Normalizer(),
                          # Birch(n_clusters=100, branching_factor=50, threshold=0.5, compute_labels=True),
                          SVR(kernel='rbf', C=1.5, epsilon=0.1, gamma='scale', max_iter=-1, tol=1e-6))
