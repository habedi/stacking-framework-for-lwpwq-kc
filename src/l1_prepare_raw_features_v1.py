...
def count_by_values(df, colname, values):
    ...
    for i, value in enumerate(values):
        ...
        fts = fts.join(tmp_df, on=global_settings.id_col, how='left')
    return fts
...
def dev_feats(df):
    ...
    feats = feats.join(count_by_values(df, 'text_change', text_changes), on=global_settings.id_col, how='left')
    feats = feats.join(count_by_values(df, 'down_event', events), on=global_settings.id_col, how='left')
    feats = feats.join(count_by_values(df, 'up_event', events), on=global_settings.id_col, how='left')
    ...
    feats = feats.join(temp, on=global_settings.id_col, how='left')
...
def get_essay_df(df):
    ...
    essay_df = essay_df.merge(temp.rename('essay'), on=global_settings.id_col)
    return essay_df
...
def product_to_keys(logs, essays):
    ...
    essays = essays.merge(tmp_df, on=global_settings.id_col, how='left')
...
def get_keys_pressed_per_second(logs):
    ...
    temp_df = temp_df.merge(temp_df_2, on=global_settings.id_col, how='left')
    temp_df['keys_per_second'] = temp_df['keys_pressed'] / ((temp_df['max_up_time'] - temp_df['min_down_time']) / 1000)
    return temp_df[['id', 'keys_per_second']]


train_logs = pl.scan_csv(global_settings.data_dir / 'train_logs.csv.gz')
...
train_feats = train_feats.merge(word_feats(train_essays), on=global_settings.id_col, how='left')
train_feats = train_feats.merge(sent_feats(train_essays), on=global_settings.id_col, how='left')
train_feats = train_feats.merge(parag_feats(train_essays), on=global_settings.id_col, how='left')
train_feats = train_feats.merge(get_keys_pressed_per_second(train_logs), on=global_settings.id_col, how='left')
train_feats = train_feats.merge(product_to_keys(train_logs, train_essays), on=global_settings.id_col, how='left')

print('< Mapping >')
train_scores = pd.read_csv(global_settings.data_dir / 'train_scores.csv.gz')
train_feats_with_scores = train_feats.merge(train_scores, on=global_settings.id_col, how='left')

print('< Testing Data >')
test_logs = pl.scan_csv(global_settings.data_dir / 'test_logs.csv.gz')
test_feats = dev_feats(test_logs)
test_feats = test_feats.collect().to_pandas()

test_logs = test_logs.collect().to_pandas()
test_essays = get_essay_df(test_logs)
test_feats = test_feats.merge(word_feats(test_essays), on=global_settings.id_col, how='left')
test_feats = test_feats.merge(sent_feats(test_essays), on=global_settings.id_col, how='left')
test_feats = test_feats.merge(parag_feats(test_essays), on=global_settings.id_col, how='left')
test_feats = test_feats.merge(get_keys_pressed_per_second(test_logs), on=global_settings.id_col, how='left')
test_feats = test_feats.merge(product_to_keys(test_logs, test_essays), on=global_settings.id_col, how='left')
...
# Write to csv files
train_feats_with_scores.to_csv(global_settings.raw_features_dir / 'raw_train_features_with_scores_v1.csv.gz',
                               index=False)
test_feats.to_csv(global_settings.raw_features_dir / 'raw_test_features_v1.csv.gz', index=False)
