...
if global_settings.use_stratified_folds or True:
    skf = StratifiedKFold(n_splits=global_settings.l3_n_folds, shuffle=True,
                          random_state=global_settings.global_seed + global_settings.random_seed_offsets[3])
else:
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(oofs_train_df, y_as_str)):
    ...
    model = TPOTRegressor(generations=10, population_size=50, verbosity=2,
                          random_state=global_settings.global_seed + global_settings.random_seed_offsets[3],
...
inferences_df.to_csv(global_settings.get_meta_model_oofs_dir() / 'submission_tpot.csv.gz', index=True)
...
recorder.add_record(model_name=script_name,
                    ...
                    random_seed=global_settings.global_seed + global_settings.random_seed_offsets[3], )
...
meta_model_df.to_csv(global_settings.get_meta_model_oofs_dir() / f'val_tpot.csv.gz', index=False)
