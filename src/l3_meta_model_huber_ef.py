...
train_essays_df = pd.read_csv(global_settings.raw_features_dir /
                              f'train_features_with_scores_v{global_settings.get_featureset_version()}.csv.gz',
                              index_col=global_settings.id_col)
train_essays_df.fillna(0, inplace=True)

test_essays_df = pd.read_csv(global_settings.raw_features_dir /
                             f'test_features_v{global_settings.get_featureset_version()}.csv.gz',
                             index_col=global_settings.id_col)
test_essays_df.fillna(0, inplace=True)

oofs_train_df = pd.merge(oofs_train_df, train_essays_df.drop([global_settings.target_col], axis=1),
                         on=global_settings.id_col, how='left')
oofs_inference_df = pd.merge(oofs_inference_df, test_essays_df,
                             on=global_settings.id_col, how='left')
...
inferences_df.to_csv(global_settings.get_meta_model_oofs_dir() / 'submission_huber_ef.csv.gz', index=True)
...
meta_model_df.to_csv(global_settings.get_meta_model_oofs_dir() / f'val_huber_ef.csv.gz', index=False)
