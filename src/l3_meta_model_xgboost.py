...
if global_settings.use_stratified_folds or True:
    skf = StratifiedKFold(n_splits=global_settings.l3_n_folds, shuffle=True,
                          random_state=global_settings.global_seed + global_settings.random_seed_offsets[4])
else:
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(oofs_train_df, y_as_str)):
    ...
    model.fit(X_train, y_train, eval_set=[(X_valid, y_valid)], early_stopping_rounds=200, verbose=100)
...
inferences_df.to_csv(global_settings.get_meta_model_oofs_dir() / 'submission_xgb.csv.gz', index=True)
...
recorder.add_record(model_name=script_name,
                    ...
                    random_seed=global_settings.global_seed + global_settings.random_seed_offsets[4], )
...
meta_model_df.to_csv(global_settings.get_meta_model_oofs_dir() / f'val_xgb.csv.gz', index=False)
